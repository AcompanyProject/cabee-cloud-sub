...
import os, random, json, time, re, csv, requests
...
# グローバル変数
WEB_HOOK_URL = "https://hooks.slack.com/services/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"
...
def trade_executor(request):
    ...
    login.operation_login(driver, USER_INFO)
    ...
    # サインによって売買する（建玉を変更する）
    try:
        operation_switch_trade(realtime_contract, sign)
    except Exception as err:
        requests.post(WEB_HOOK_URL, data = json.dumps({
            'text': '売買操作中にエラーが生じました: ' + str(err),
...
def operation_get_realtime_deposit():
    ...
    try:
        ...
        requests.post(WEB_HOOK_URL, data = json.dumps({
            'text': 'ログイン後の操作に失敗しました。以下を確認してください： 1. 設定したログインID, PASSは正しいか? 2. 松井証券のサイトログイン後に契約更新等のタスクが未完了か？ Error: ' + str(err),
            'username': u'エラー', 'icon_emoji': u':fire:', 'link_names': 1,
        }))
    ...
    # リアルタイム先物評価損益の取得
    REALTIME_PROFIT_LOSS = WebDriverWait(driver, 10).until(EC.visibility_of_element_located((By.ID, 'powerInfoFutOpRealFutUnrealized'))).text
    REALTIME_PROFIT_LOSS = REALTIME_PROFIT_LOSS.translate(str.maketrans({',':'', '円':''}))

    print('損失率: ' + REALTIME_PROFIT_LOSS/REALTIME_SHEET_NUM)
    if REALTIME_PROFIT_LOSS/REALTIME_SHEET_NUM > -40000:
        # 4%以上の損失発生時に一応通知する
        requests.post(WEB_HOOK_URL, data = json.dumps({
            'text': '4%を超える損失が発生しています。システムが正常に損切りするかどうか確認してください。しなければ手動で損切りしてください。',
            'username': u'損失の発生', 'icon_emoji': u':rotating_light:', 'link_names': 1,
            "attachments": [{ "image_url": "order-complete.png"}]
        }))
...
def operation_repayment_order(purpose, is_buy_sign):
    ...
    driver.save_screenshot('log/images/click-comfirm-btn.png')
...
def common_operation_order(order_kind, order_kind2, operation):
    ...
    time.sleep(3)
    requests.post(WEB_HOOK_URL, data = json.dumps({
        'text': '注文が完了しました', 'username': u'サイン発生', 'icon_emoji': u':cat:', 'link_names': 1,
        "attachments": [{ "image_url": "order-complete.png"}]
    }))
...
# 注文切り替え
def operation_switch_trade(realtime_contract, sign):
    # テスト
    ...
    if purpose == 'new_order':
        print('新規注文します')
        requests.post(WEB_HOOK_URL, data = json.dumps({ 'text': '新規注文します', 'username': u'サイン発生', 'icon_emoji': u':cat:', 'link_names': 1 }))
        ...
        driver.save_screenshot('log/images/new-order-complete.png')
    elif purpose == 'repayment_order':
        print('返済注文します')
        requests.post(WEB_HOOK_URL, data = json.dumps({ 'text': '返済注文します', 'username': u'サイン発生', 'icon_emoji': u':cat:', 'link_names': 1 }))
        WebDriverWait(driver, 20).until(EC.visibility_of_element_located((By.CSS_SELECTOR, '.btn-menu-fut-op-speed-order'))).click()
        time.sleep(5)
        operation_repayment_order(purpose, is_buy_sign)
        driver.save_screenshot('log/images/repayment-order-complete.png')
    elif purpose == 'repayment_and_new_order':
        print('返済注文と新規注文をします')
        requests.post(WEB_HOOK_URL, data = json.dumps({ 'text': '返済注文と新規注文をします', 'username': u'サイン発生', 'icon_emoji': u':cat:', 'link_names': 1 }))
        WebDriverWait(driver, 20).until(EC.visibility_of_element_located((By.CSS_SELECTOR, '.btn-menu-fut-op-speed-order'))).click()
        time.sleep(5)
        operation_repayment_order(purpose, is_buy_sign)
        driver.save_screenshot('log/images/repayment-order-complete.png')
